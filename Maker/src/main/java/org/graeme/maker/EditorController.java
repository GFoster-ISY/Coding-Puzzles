...
public class EditorController {
	...
	private final String resoursePath = "Maker/src/main/resources/org/graeme/maker";
    ...
    private Node createBooleanGraphic(boolean required){
        ...
        try {
        	if (required) {
        		img = new Image(new FileInputStream(resoursePath + "/green-checkmark.jpg"));
        	} else {
        		img = new Image(new FileInputStream(resoursePath + "/red-cross.jpg"));
        	}
    ...
    private void getAllProblemFiles() {
		File folder = new File(resoursePath);
		...
		for (int i = 0; i < listOfFiles.length; i++) {
	        ...
	        try {
	        	Object obj = parser.parse(new FileReader(resoursePath+"/"+listOfFiles[i].getName()));
